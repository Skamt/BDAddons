...
export default function Artist({ artists }) {
	...
	return (
		<Popout renderPopout={e => <ContextMenu.Menu onClose={e.closePopout}>{ContextMenu.buildMenuChildren(menu)}</ContextMenu.Menu>}>
			{e => (
				<div
					onClick={e.onClick}
					className="outline pointer spotify-player-artist ellipsis">{`by ${artists[0].name}`}</div>
			)}
		</Popout>
	);
}
